...
현장에서 앱을 켜두면 일정 시간 뒤 화면이 자동으로 꺼졌다. 화면이 꺼지면 안드로이드가 앱을 백그라운드로 넘기면서 웹소켓 연결을 끊는 것으로 보였다. MQTT 쪽은 원인이 더 직접적이었다 — 백그라운드 진입 시 연결을 명시적으로 끊어버리는 코드가 있어서, 화면이 꺼지자마자 연결 상태 확인 자체가 멈춰버렸다. 실제로 테스트해보니, 백그라운드에서도 연결을 최대한 지속시키는 쪽이 사용성에 훨씬 유리했다.
...
2. 백그라운드 진입 시 MQTT 연결을 끊던 코드를 제거해, 앱이 백그라운드에 있어도 연결 상태 확인이 어느 일정시간정도 OS가 허락하는 시간까지는 계속되도록 했다.
3. 웹소켓 재연결을 즉시 시도하던 걸, 약간의 텀을 두고 재시도하도록 바꿨다.
